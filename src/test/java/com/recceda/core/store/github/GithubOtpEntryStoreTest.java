...
class GithubOtpEntryStoreTest {
  ...
  // This is an end-to-end test that requires a valid GitHub API token with repository creation and
  ...
  void e2eWithReccedaOtp() throws Exception {
    ...
    CompletableFuture<Void> future1 =
        reccedaOtp.generateOtp(
            "test-user-1",
            OtpConfig.builder().build(),
            (k, o) -> {
              otp1.set(o);
              key1.set(k);
    ...
    Assertions.assertTrue(
        reccedaOtp.verifyOtp(key1.get(), otp1.get()).get(), "The first OTP should be valid.");
    ...
    CompletableFuture<Void> future2 =
        reccedaOtp.generateOtp(
            "test-user-2",
            OtpConfig.builder().ttlMillis(1).build(),
            (k, o) -> {
              otp2.set(o);
              key2.set(k);
              return CompletableFuture.completedFuture(null);
    ...
    Assertions.assertFalse(
        reccedaOtp.verifyOtp(key2.get(), otp2.get()).get(), "The second OTP should have expired.");
  ...
  void e2eTest() throws Exception {
    ...
    Assertions.assertTrue(
        otpStore.verifyOtp("test-user", "123456").get(), "The OTP should be valid.");
    Assertions.assertFalse(
        otpStore.verifyOtp("test-user", "654321").get(), "The OTP should be invalid.");

    otpStore.invalidateOtp("test-user").get();

    Assertions.assertFalse(
        otpStore.verifyOtp("test-user", "123456").get(),
        "The OTP should be invalid after invalidation.");
