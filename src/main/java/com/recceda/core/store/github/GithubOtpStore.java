...
public class GithubOtpStore implements OtpStore {
  ...
  public GithubOtpStore(String githubToken, String otpStoreName)
    ...
    this.repository =
        this.repositoryAction.createRepositoryForAuthenticatedUser(
            this.createOtpRepository(otpStoreName));
  }
  ...
  public CompletableFuture<Void> storeOtp(String key, String plainOtp, long ttlMillis) {
    return CompletableFuture.runAsync(
        () -> {
          OtpEntry otp =
              OtpEntry.builder()
                  .otpHash(HashingUtil.hashOtp(plainOtp))
          ...
          try {
            fileAction.createFile(
                this.createFileRequestForOtp(otp), this.repository, this.generateOtpFileName(key));
          } catch (Exception e) {
  ...
  public CompletableFuture<Boolean> verifyOtp(String key, String plainOtp) {
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            OtpEntry otp =
                fileAction.getFileContents(
                    this.repository.getOwner().getLogin(),
                    this.repository.getName(),
                    this.generateOtpFileName(key),
                    OtpEntry.class);
            String otpHash = HashingUtil.hashOtp(plainOtp);
            String otpHashFromStore = otp.getOtpHash();
            if (!otpHash.equals(otpHashFromStore)
                || System.currentTimeMillis() > otp.getExpiryTime()) {
              String sha =
                  fileAction
                      .getFileContents(
                          this.repository.getOwner().getLogin(),
  ...
  public CompletableFuture<OtpEntry> getOtpEntry(String key) {
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            return fileAction.getFileContents(
                this.repository.getOwner().getLogin(),
                this.repository.getName(),
                this.generateOtpFileName(key),
                OtpEntry.class);
        ...
        () -> {
          try {
            ...
            fileAction.deleteFile(
                this.createDeleteFileRequest(key, sha),
                this.repository.getOwner().getLogin(),
                this.repository.getName(),
                this.generateOtpFileName(key));
          } catch (Exception e) {
  ...
  private CreateFileRequest createFileRequestForOtp(OtpEntry otpEntry)
      throws JsonProcessingException {
    String message = LocalDate.now() + " " + " " + otpEntry.getKey();
    Committer committer =
        Committer.builder().name(this.getClass().getName()).email("test@example.com").build();

    return new CreateFileRequest(otpEntry, committer, message);
  }

  private DeleteFileRequest createDeleteFileRequest(String key, String sha)
      throws JsonProcessingException {
    String message = LocalDate.now() + " " + " " + key + " deleted";
    Committer committer =
        Committer.builder().name(this.getClass().getName()).email("test@email.com").build();
  ...
  public void destroy() throws ExecutionException, InterruptedException {
    repositoryAction.deleteRepositoryForAuthenticatedUser(
        this.repository.getOwner().getLogin(), this.repository.getName());
  }
}
